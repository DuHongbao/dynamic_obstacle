...
void execCallback(const ros::TimerEvent &e) {
  ...
  case REPLAN_TRAJ: {
    ...
    t_cur = t_delta + t_cur;
    start_pt = getPos(t_cur);//change star_pt
    cout<<"**************change start_pt**************"<<start_pt<<endl;
...
bool trajGeneration() {
   ...
   *
   * **/
  cout<<"*******trajGenerationstart_pt******"<<start_pt<<endl;
...
void trajOptimization(Eigen::MatrixXd path) {
  cout<<"********************trajOPtimization Beginning***********"<<endl;
...
VectorXd timeAllocation(MatrixXd Path) {
  cout<<"********************timeAllocation is OK***********"<<endl;
  VectorXd time(Path.rows() - 1);
  double Path_length=0,t=0;
  for(int i=0;i<Path.rows()-1;i++){
    Path_length += (Path.row(i+1)-Path.row(i)).norm();
  }

  for(int i=0;i<Path.rows()-1;i++){
    t += (Path.row(i+1)-Path.row(i)).norm()/Path_length;
    time[i]=t;
  }
  time *= Path_length/1;////////////////////////////////////
  cout<<time<<"time:"<<endl;
