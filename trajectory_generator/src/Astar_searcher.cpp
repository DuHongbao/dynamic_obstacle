...
double AstarPathFinder::getHeu(GridNodePtr node1, GridNodePtr node2) {
  ...
  double h;
  h = (node1->coord - node2->coord).lpNorm<1>();//Manhattan distance
  //h = (node1->coord - node2->coord).norm();//Euclidean distance
  //h = (node1->coord - node2->coord).lpNorm<Infinity>();//Infinity distance
  return h;
}
...
void AstarPathFinder::AstarGraphSearch(Vector3d start_pt, Vector3d end_pt) {
  ...
  cout<<"**************Astar start_pt**************"<<start_pt<<endl;
   ...
   *
   * **/
  currentPtr = openSet.begin()->second;//20210127
  currentPtr->id = -1;//20210127
  ...
  while (!openSet.empty()) {
    //cout<<"jinru while xunhuan"<<endl;
        ...
        *
        */
		std::multimap<double,GridNodePtr>::iterator it = openSet.begin();
        (*it).second->id=-1;
		openSet.erase(it);
		//cout<<"shanchu zuixiaodian"<<endl;
        //cout<<openSet.size()<<endl;
            ...
            */
            ...
            if(neighborPtr -> id == 0){ //discover a new node, which is not in the closed set and open set
                ...
                */
                ...
                //cout<<"openSet de yuansu"<<neighborPtr->fScore<<endl;
                ...
                *        
                */
                neighborPtr->fScore = inf;//
                neighborPtr->gScore = inf;//
                continue;
            }
        }  
        currentPtr = openSet.begin()->second; 
        //cout<<"current Ptr is:\n"<<currentPtr->index<<" "<<currentPtr->gScore<<endl; 
...
vector<Vector3d> AstarPathFinder::getPath() {
  ...
  vector<GridNodePtr> gridPath;
    ...
    ** 
    */
    GridNodePtr end_Ptr = GridNodeMap[goalIdx(0)][goalIdx(1)][goalIdx(2)];//
    gridPath.push_back(end_Ptr);/////
    //gridPath.push_back(terminatePtr);//endPtr;
    Vector3i start_index = terminatePtr->index;
    //Vector3i start_index = coord2gridIndex(startPtr);
    //cout<<terminatePtr->index<<endl;
    //int i=0;
    end_Ptr->id = -1;
    cout<<"**********************getPath***********"<<terminatePtr->coord<<endl;
    while ( end_Ptr->index != start_index){
    	 if(end_Ptr->id == -1){
          end_Ptr = end_Ptr -> cameFrom;
    	    gridPath.push_back(end_Ptr);
...
vector<Vector3d> AstarPathFinder::pathSimplify(const vector<Vector3d> &path,
  ...
  vector<Vector3d> subPath,subPath_1, subPath_2;
  cout<<"**********this is RDP**********"<<endl;
  //STEP 2.1:  implement the RDP algorithm
  double dmax = 0;
  int index = 0;
  int end = path.size();
  for (int i = 2;i<end-1;i++){
    Vector3d p=path[i],p1=path[0],p2=path[path.size() - 1];
    double d=perpendicularDistance(p, p1, p2);
    if(d>dmax)
    {
      index = i;
      dmax = d;
  ...
  if (dmax > path_resolution){
    vector<Vector3d> pre_part, next_part;
    for (int i = 0; i<= index; ++i) pre_part.push_back(path[i]);
    for (int i = index; i< path.size(); ++i) next_part.push_back(path[i]);
    //Recursive call
    vector<Vector3d> resultList1 = pathSimplify(pre_part, path_resolution);
    vector<Vector3d> resultList2 = pathSimplify(next_part, path_resolution);

    //combine
    subPath.insert(subPath.end(), resultList1.begin(), resultList1.end());
    subPath.insert(subPath.end(), resultList2.begin()+1, resultList2.end());
  }
  else{
...
double AstarPathFinder::perpendicularDistance(const Vector3d& p, const Vector3d& line_p1, Vector3d& line_p2){
    Vector3d n=line_p2-line_p1,P_n;
    P_n=line_p1+n*(n.dot(p-line_p1)/n.squaredNorm());
    double d=(P_n-p).norm();
    return d;
